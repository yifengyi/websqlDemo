...
(function(jQuery) {
    ...
    JQueryDatabase.prototype.tables = function(callback) {
        var mySuccessCallback = function(transaction, resultSet) {
            ...
            for (i = 0; i < len; i = i + 1) {
                ...
                console.log('table name---------' + item.name);
    ...
    /**
     * Inserts  a new row into a table if exist replace it
     *
     * @function
     * @memberOf JQueryDatabase
     *
     ...
     * @returns {JQueryDatabase}
     */
    JQueryDatabase.prototype.insertOrReplace = function(tableName, params) {
        var values = [];
        var columns = [];
        var placeholders = [];
        ...
        var sql = "INSERT OR REPLACE INTO " + tableName + " (" + columns.join(",") + ") VALUES (" + placeholders.join(",") + ")";
     ...
     * @returns {JQueryDatabase}
     */
    JQueryDatabase.prototype.batchInsert=function(tableName, params){
        var values = [];
        var columns = [];
        var placeholders = [];
        ...
        if (params.data instanceof Array) {
            jQuery.each(params.data[0], function(index, obj) {
                placeholders.push("?");
                // values.push(obj);
                columns.push(index);
            });
            ...
            jQuery.each(params.data,function(index,obj){
                childSql.push('select '+placeholders.join(','));
            ...
            var sql = "INSERT INTO " + tableName + " (" + columns.join(",") + ")" + childSql.join(" union ");
    ...
    JQueryDatabase.prototype._execute = function(sql, args, successCallback, errorCallback) {
        ...
        this.database.transaction(caller, successCallback, errorCallback);
    };
    JQueryDatabase.prototype._executeIn = function(sql, args, successCallback, errorCallback) {
        var caller = function(transaction) {
            args = args || [];
    ...
    jQuery.db = function(shortName, version, displayName, maxSize, creationCallback) {
        var db;
        console.log('pre open db');
        if (!openDatabase) {
            db = undefined;
            console.log('pre open db 1');
        } else {
            var openDB = openDatabase(shortName, version, displayName, maxSize, function(openDB) {
            ...
            if (openDB) {
                console.log('pre open db 2');
                db = new JQueryDatabase(openDB);
            } else {
                console.log('pre open db 3');
                db = undefined;
            }
        }
